...
class IconResourceIT {
    ...
    public static Icon createEntity(EntityManager em) {
        Icon icon = new Icon().url(DEFAULT_URL).name(DEFAULT_NAME);
        return icon;
    }
    ...
    public static Icon createUpdatedEntity(EntityManager em) {
        Icon icon = new Icon().url(UPDATED_URL).name(UPDATED_NAME);
        return icon;
    }
    ...
    void createIcon() throws Exception {
        ...
        restIconMockMvc
            .perform(
                post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(icon))
            )
    ...
    @Test
    @Transactional
    void getAllIconsByCategoryIsEqualToSomething() throws Exception {
        // Initialize the database
        iconRepository.saveAndFlush(icon);
        ...
        // Get all the iconList where category equals to categoryId
        defaultIconShouldBeFound("categoryId.equals=" + categoryId);

        // Get all the iconList where category equals to (categoryId + 1)
        defaultIconShouldNotBeFound("categoryId.equals=" + (categoryId + 1));
    }

    @Test
    @Transactional
    void getAllIconsByWalletIsEqualToSomething() throws Exception {
        // Initialize the database
        iconRepository.saveAndFlush(icon);
        Wallet wallet = WalletResourceIT.createEntity(em);
        em.persist(wallet);
        em.flush();
        icon.addWallet(wallet);
        iconRepository.saveAndFlush(icon);
        Long walletId = wallet.getId();

        // Get all the iconList where wallet equals to walletId
        defaultIconShouldBeFound("walletId.equals=" + walletId);

        // Get all the iconList where wallet equals to (walletId + 1)
        defaultIconShouldNotBeFound("walletId.equals=" + (walletId + 1));
    }
    ...
    void putNewIcon() throws Exception {
        ...
        updatedIcon.url(UPDATED_URL).name(UPDATED_NAME);
    ...
    void putWithMissingIdPathParamIcon() throws Exception {
        ...
        restIconMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(icon))
            )
    ...
    void fullUpdateIconWithPatch() throws Exception {
        ...
        partialUpdatedIcon.url(UPDATED_URL).name(UPDATED_NAME);
    ...
    void deleteIcon() throws Exception {
        ...
        // Delete the icon
        restIconMockMvc
            .perform(delete(ENTITY_API_URL_ID, icon.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
