...
@Component({
  ...
  template: `
    <div class="example-header">
      <button mat-icon-button class="example-double-arrow" (click)="previousClicked('year')"><<</button>
      <button mat-icon-button (click)="previousClicked('month')"><</button>
      <span class="example-header-label">{{ periodLabel }}</span>
      <button mat-icon-button (click)="nextClicked('month')">></button>
      <button mat-icon-button class="example-double-arrow" (click)="nextClicked('year')">>></button>
    </div>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
