...
export class NestableComponent implements OnInit, OnDestroy {
  ...
  constructor(private ref: ChangeDetectorRef, private renderer: Renderer2, private el: ElementRef, private zone: NgZone) {}
  ...
  private _move(event) {
    ...
    if (!this._mouse.dirAx || isNewRoot) {
      ...
      // check depth limit
      depth = this.dragDepth - 1 + helper._getParents(this.pointEl, this.el.nativeElement.querySelector(this.options.listNodeName)).length;
  ...
  public dragStop(event) {
    ...
    if (this.dragEl) {
      ...
      let changedElementPosition =
        this._dragIndex !== Array.prototype.indexOf.call(this._placeholder.parentElement.children, this._placeholder);
      ...
      if (placeholderContainer === null) {
        this.list.splice(Array.prototype.indexOf.call(this._placeholder.parentElement.children, this._placeholder), 0, {
          ...this.dragModel,
        });
      } else {
        // palceholder nested
