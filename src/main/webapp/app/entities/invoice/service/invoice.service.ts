...
export class InvoiceService {
  ...
  protected convertDateFromClient(invoice: IInvoice): IInvoice {
    return Object.assign({}, invoice, {
      dateCreated: invoice.dateCreated?.isValid() ? invoice.dateCreated.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((invoice: IInvoice) => {
        invoice.dateCreated = invoice.dateCreated ? dayjs(invoice.dateCreated) : undefined;
      });
    }
    return res;
  }
